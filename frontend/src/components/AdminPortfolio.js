...
const ItemType = "PORTFOLIO_ROW";

const SortableTableRow = ({ portfolio, index, moveRow, apiUrl, onEdit, onDelete }) => {
  ...
  return (
    <TableRow
      ...
      style={{ opacity: isDragging ? 0.5 : 1, backgroundColor: isDragging ? "#f5f5f5" : "inherit" }}
    >
      <TableCell sx={{ fontWeight: "medium", cursor: "move", width: 50 }}>
...
const AdminPortfolio = () => {
  ...
  const moveRow = (dragIndex, hoverIndex) => {
    const dragPortfolio = portfolios[dragIndex];
    const newPortfolios = [...portfolios];
    newPortfolios.splice(dragIndex, 1);
    newPortfolios.splice(hoverIndex, 0, dragPortfolio);
